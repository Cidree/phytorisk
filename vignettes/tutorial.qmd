...
::: callout-note
To install the package and run this tutorial on your own machine, follow
[these instructions](index.html).
:::
...
A quick look at the study area and infection foci including the
elevation gradient from the digital elevation model:
...
And now the study area and infection foci including the tree cover:

```{r}
#| label: fig-data2
#| code-fold: true
ggplot() +
  geom_spatraster(data = as.factor(trees_sr)) +
  scale_fill_manual(
    values =c("transparent","#228B22"),
    name     = "Tree cover"
  ) +
  geom_sf(
    data = aoi_sf, 
...
ggplot() +
  ...
  new_scale_fill() +
  geom_spatraster(data = as.factor(trees_sr),alpha=0.4) +
  scale_fill_manual(
    values =c("transparent","#228B22"),
    name     = "Tree cover"
  ) +
...
This function requires the output of `mec_soilwater()` as an input, so
...
risk surface using `phytorisk_ensemble()`. By default, equal weights are
assigned to each active mechanism:
...
`terra::focal()` with `fun = "sum"`, which is equivalent to a weighted
mean.
